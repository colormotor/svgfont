...
def load_font(svg_font_file, tol=0.5):
    """
    Parsing code mainly written by ChatGPT
    """
    if not svg_font_file.endswith('.svg'):
        svg_font_file = os.path.join(os.path.dirname(__file__), 'hershey', svg_font_file+'.svg')
    ...
    units_per_em = float((ff.get('units-per-em') if ff is not None else '1000') or '1000')

    glyphs = {}
    missing = None

    mg = font_elem.find('svg:missing-glyph', ns) if ns else font_elem.find('missing-glyph')
    if mg is not None:
        d = mg.get('d')
        if d is not None:
    ...
    glyph_elems = font_elem.findall('svg:glyph', ns) if ns else font_elem.findall('glyph')
...
def text_polylines(text, font, size=1.0, pos=[0, 0], box=None, padding=0, letter_spacing=0.0, line_height=1.25, **kwargs):
    """
    Compose a string into positioned polylines.

    Args:
        text: Unicode string to render.
        font: StrokeFont loaded with load_svg_font_polylines.
        size: Target height in user units; scale is size / units_per_em.
        letter_spacing: extra advance added per glyph (user units).
        line_height: multiple of size to move down on newline ('\\n').

    Returns:
        list of np.ndarray: polylines in user units with Y increasing upwards.
    """
    if type(font)==str:
